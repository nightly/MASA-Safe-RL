...
This page introduces the core ideas behind reinforcement learning, safe RL, and how MASA builds a canonical, modular interface for handling safety constraints ranging from simple cost budgets to temporal logic specifications.
...
The agent's goal is to learn a policy $\pi(a \mid o)$ that maximizes expected cumulative reward over time.
...
Most modern RL code is built around the Gymnasium API, which standardizes how agents interact with environments.
...
- Episodes end when `terminated` (task finished or failure) or `truncated` (time limit).
...
In many applications, maximizing reward is not enough.
...
In a CMDP, the agent optimizes reward while keeping expected cost below a budget:
...
2. Standardize safety signals across methods
...
![Overview of the abstraction stack used in MASA](../_static/img/path355.svg)
...
- [Constraints](../Common/Constraints) - Learn how different constraints are handled in MASA.
- [Wrappers](../Common/Wrappers) - See how wrappers provide a convenient interface for managing constraints.
